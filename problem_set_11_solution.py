...
# WARN: Local testing only; comment out for Gradescope auto grader testing
import five_oh_six_solution as utl

# WARN: Uncomment for Gradescope auto grader testing
...
def board_passengers(starship, passengers):
    ...
    # Preferred: stop value may exceed actual length of list)
    starship["passengers_on_board"] = passengers[: starship["max_passengers"]]
    return starship
...
def convert_gravity_value(value):
    ...
    try:
        val = value.strip().lower()  # case-insensitive
        if val.find("standard") > -1:
            return utl.convert_to_float(val.replace("standard", ""))
        else:
            return utl.convert_to_float(val)
    except:
...
def create_person(data, planets=None):
    ...
    if planets:
        mandalorian_planet = get_mandalorian_data(planets, data.get("homeworld"))
        if mandalorian_planet:
            homeworld = create_planet(mandalorian_planet)
    elif data.get("homeworld"):
        try:
            swapi_homeworld = get_swapi_resource(SWAPI_PLANETS, {"search": data.get("homeworld")})["results"][
                0
            ]
            homeworld = create_planet(swapi_homeworld)
        except:
            homeworld = create_planet(get_swapi_resource(data.get("homeworld")))
    else:
        homeworld = None
...
def main():
    ...
    assert utl.convert_to_int([618, 664]) == [618, 664]
    ...
    # Problem 3.2.1  Test < create_planet >
    ...
    # PROBLEM 4
    # Problem 4.1.1 Test < create_droid >
    ...
    mando = create_person(mandalorian_din_djarin, planets=mandalorian_planets)

    # Problem 4.3.2 Write to file
    utl.write_json("stu-mando.json", mando)

    # PROBLEM 5
    # Problem 5.1.1 Test < create_starship >
    ...
    # Problem 5.3.1 Test < board_passengers >
    razor_crest = board_passengers(razor_crest, [mando])

    print(f"\n5.2.1 razor crest passengers on board = {razor_crest['passengers_on_board']}")
    ...
    # 7.1.1 Get Kuiil
    kuiil = create_person(get_mandalorian_data(mandalorian_people, "Kuiil"), planets=mandalorian_planets)
    ...
    print(f"\n9.2 razor crest visited planets = {razor_crest['planets_visited']}")

    # Problem 9.2
    utl.write_json("stu-razor_crest_departs.json", razor_crest)

    # PERSIST util.cache (DO NOT COMMENT OUT)
